  ...
  return (
    <section className="rounded-3xl border border-white/10 bg-zinc-900 p-6 sm:p-8">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {state.players.map((hand, playerIndex) => {
          const isCurrentPlayer =
            state.turn.currentPlayer === playerIndex;
          ...
          const drinks =
            state.drinks[playerIndex] ?? 0;

          const jokers =
            state.memory.jokers[playerIndex] ?? 0;
